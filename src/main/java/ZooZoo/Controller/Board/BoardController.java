...
public class BoardController {
    ...
    public String goToShareBoardList(Model model, @RequestParam(defaultValue = "1") int page) {
        String area = request.getParameter("option");
        String aa = area;
        System.out.println("aaaaaaaaaaaaaaaaaaaaaaaaaaaaa" + aa);
        if(area == null) { // 처음 페이지 열 때
            ShareDTO shareDTO = new ShareDTO();
            ArrayList<ShareDTO> shareDTOS = new ArrayList<>();
            ArrayList<String> petshop = shareService.Share(aa);
